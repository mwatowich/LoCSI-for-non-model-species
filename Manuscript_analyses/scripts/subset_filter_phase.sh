...
# Filter retain samples with missingness, remove sites with high fraction missing 
...
java -Xmx50g -jar $path_beagle nthreads=24 impute=false gt=chr${chrom}.filtered.vcf.gz out=chr${chrom}.phased chrom=${chrom}
tabix chr${chrom}.phased.vcf.gz
